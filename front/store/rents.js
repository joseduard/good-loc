...
const getters= {
  ...
  const actions= {
    fetchReserved({ commit, rootState }, page) {
      ...
      // Exemple :
      this.$axios.get(`api/user/account/rent/${this.$auth.$storage.getUniversal('user').id}/reserved?pageSize=8&page=${page}`)
        .then((res) => {
    ...
    //   const userId = UserId;
      ...
      // Faites votre appel API pour récupérer les données de location terminées
      // Utilisez commit pour appeler la mutation appropriée et mettre à jour l'état
      // Exemple :
      this.$axios.get(`api/user/account/rent/${this.$auth.$storage.getUniversal('user').id}/closed?pageSize=8&page=${page}`)
        .then((res) => {

...
export const login = async (req, res) => {
  try {
    ...
    let token = generateToken({ id: user.id, pseudo: user.pseudo },  TOKEN_EXPIRY_TIME);
...
export const logout = async (req, res) => {
  try {
    const user = await logoutUser(req.body.userId);
...
export const forgotPassword = async (req, res) => {
  try {
    const {user, token} = await processForgotPassword(req.body.email);

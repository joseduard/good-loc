...
const Publishers = require("../models/Publishers.js")(db.sequelize);
const Categories = require("../models/Categories.js")(db.sequelize); // Ajout de l'importation pour le modèle "Categories"
const Mechanics_Type = require("../models/Mechanics_Type.js")(db.sequelize); // Ajout de l'importation pour le modèle "Mechanics_Type"
...
async function fetchGameDetails(idGame) {
  ...
  try {
    ...
    if (!existingGame) {
      if (gameData.id) {
        if (gameData.primary_publisher && gameData.primary_publisher.id !== undefined) {
          const existingPublisher = await Publishers.findOne({ where: { id: gameData.primary_publisher.id } });
        ...
        if (gameData.categories[0] !== undefined) {
          const existingCategory = await Categories.findOne({ where: { id: gameData.categories[0].id } });

          if (!existingCategory) {
            const searchId = gameData.categories[0].id;
            axios.get('https://api.boardgameatlas.com/api/game/categories?pretty=true&client_id=JLBr5npPhV')
              .then(response => {
                const categories = response.data.categories;
                const category = categories.find(category => category.id === searchId);
        ...
        if (gameData.mechanics[0] !== undefined) {
          const existingMechanic = await Mechanics_Type.findOne({ where: { id: gameData.mechanics[0].id } });

          if (!existingMechanic) {
            const searchMechanics = gameData.mechanics[0].id;
            axios.get('https://api.boardgameatlas.com/api/game/mechanics?pretty=true&client_id=JLBr5npPhV')
              .then(response => {
                const mechanics = response.data.mechanics;
                const mechanic = mechanics.find(mechanic => mechanic.id === searchMechanics);
                if (mechanic) {
                  ...
                  console.log('Mécanique non trouvée pour l\'ID:', searchMechanics);
                }
              });
          }
        }
        ...
        await Games.create({
          ...
          name: gameData.name,
          publisher_id: gameData.primary_publisher ? gameData.primary_publisher.id : null,
          description: gameData.description,
          category_id: gameData.categories[0] ? gameData.categories[0].id : null,
          mechanics_type_id: gameData.mechanics[0] ? gameData.mechanics[0].id : null,
